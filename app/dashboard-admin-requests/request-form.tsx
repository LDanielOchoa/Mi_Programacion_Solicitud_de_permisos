...
export default function PermitRequestForm() {
  ...
  const handleCodeSelect = async (selectedCode: string) => {
    ...
    setIsLoading(true)
    try {
      const response = await fetch(`solicitud-permisos.sao6.com.co/api/user/${selectedCode}`)
      if (!response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('solicitud-permisos.sao6.com.co/api/new-permit-request', {
        method: 'POST',
        headers: {
      ...
      const approvalResponse = await fetch(`solicitud-permisos.sao6.com.co/api/update-approval/${result.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
                ...
                <div className="flex space-x-2" style={{ width: 'max-content' }}>
                  {dates.map((date, index) => (
                    <Button
                      ...
                      className={`p-2 h-auto flex flex-col items-center justify-center min-w-[60px] ${selectedDates.some(d => isSameDay(d, date)) ? 'bg-green-500 text-white' : ''
