...
export async function searchEmployees(query: string, signal?: AbortSignal) {
    try {
        ...
        const response = await fetch(`https://solicitud-permisos.sao6.com.co/api/employees/search?q=${encodeURIComponent(query)}`, {
            headers: {
                Authorization: `Bearer ${token}`,
                "Content-Type": "application/json",
            },
...
export async function updateUserPhone(phone: string) {
    try {
        ...
        const response = await fetch("https://solicitud-permisos.sao6.com.co/api/user/phone", {
            method: "PUT",
            headers: {
                Authorization: `Bearer ${token}`,
                "Content-Type": "application/json",
