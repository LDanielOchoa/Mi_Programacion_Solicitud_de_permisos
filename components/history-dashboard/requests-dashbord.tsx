...
export default function RequestDashboard() {
  ...
  const fetchFilterOptions = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`https://solicitud-permisos.sao6.com.co/api/admin/filter-options`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const fetchRequests = useCallback(
    async (page = 1, retryCount = 0) => {
      ...
      try {
        ...
        const url = `https://solicitud-permisos.sao6.com.co/api/admin/requests?${params.toString()}`
          ...
          <div className="flex flex-col space-y-6 lg:space-y-0 lg:flex-row lg:items-center lg:justify-between lg:space-x-8">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center space-y-3 sm:space-y-0 sm:space-x-4">
              <motion.button
                ...
                className={`relative flex items-center justify-center px-4 sm:px-6 py-2 sm:py-3 border-2 rounded-lg sm:rounded-xl transition-all font-bold text-sm sm:text-base shadow-lg ${
                          ...
                          <div className="grid grid-cols-1 gap-2 sm:gap-3">
                            {filterOptions.statuses.map((status) => (
                              <motion.button
                                ...
                                className={`p-3 sm:p-4 rounded-lg sm:rounded-xl border-2 transition-all duration-300 text-left font-medium text-sm sm:text-base ${
